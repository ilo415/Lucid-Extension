...
{
  // [BRAIN]: leaked into the REPLY prose below a well-formed block — this is
  // the case that used to escape detection (well-formed block, native box,
  // reply text never scanned). The dialog rebuild must strip it from `rest`.
  const text = `<thinking>\n1) plan\n2) write\n</thinking>\n\`\`\`\n\n[BRAIN]:\nShe walked across the room and stopped by the window.\n"That was a long night," she said.`;
  const r = normalizeThinkingBlock(text);
  check('11d [BRAIN]: in reply → repaired', r.repaired, true);
